    ...
    : QObject(parent), clientSequenceNumber(0), serverSequenceNumber(0)
{
    ...
    clientIdentifier = QString::number(socketDescriptor);
...
void ClientHandler::disconnected()
{
    qDebug() << "Client" << clientIdentifier << "disconnected";
    emit finished();
}
...
{
    if (receivedPacket.flags & 0b00000001) { // SYN packet
        qDebug() << "Client" << clientIdentifier << "sent SYN";
        sendSynAck();
    } else if (receivedPacket.flags & 0b00000010) { // ACK packet
        qDebug() << "Client" << clientIdentifier << "sent ACK";
        sendAckForEstablishment(receivedPacket.sequenceNumber);
    } else if (receivedPacket.flags & 0b11111100) { // Data packet
        qDebug() << "Client" << clientIdentifier << "sent data with sequence number" << receivedPacket.sequenceNumber;
        if (validatePacket(receivedPacket)) {
            qDebug() << "Client" << clientIdentifier << "data:" << QString::fromUtf8(receivedPacket.data);
            sendAckForData(receivedPacket.sequenceNumber);
        } else {
            qDebug() << "Client" << clientIdentifier << "sent invalid packet, ignoring.";
        }
    } else if (receivedPacket.flags & 0b11111110) { // Data ACK packet
        qDebug() << "Client" << clientIdentifier << "sent ACK for data segment with sequence number" << receivedPacket.sequenceNumber;
    } else {
        qDebug() << "Client" << clientIdentifier << "sent unknown packet, ignoring.";
    }
}

void ClientHandler::sendSynAck()
{
    qDebug() << "Sending SYN-ACK to client" << clientIdentifier;
    quint8 flags = 0b00000011;
...
void ClientHandler::sendAckForData(quint32 sequenceNumber)
{
    qDebug() << "Sending ACK for data segment with sequence number" << sequenceNumber << "to client" << clientIdentifier;
    quint8 flags = 0b11111110;
...
void ClientHandler::sendAckForEstablishment(quint32 sequenceNumber)
{
    qDebug() << "Sending ACK for establishment to client" << clientIdentifier;
    quint8 flags = 0b11111100;
